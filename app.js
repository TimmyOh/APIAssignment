...
const PLACES_API_KEY = 'AIzaSyDGIzbHC3ajCFUN4EfOFaoB40PqzuTF49Y';
var filteredResults;
...
server.get('/searchingpage', (req, res) => {
  res.render('searchingpage.hbs');
});
...
server.post('/getplaces', (req, res) => {
  ...
  const locationReq = `https://maps.googleapis.com/maps/api/geocode/json?address=${addr}&key=AIzaSyDGIzbHC3ajCFUN4EfOFaoB40PqzuTF49Y`;
...
server.get('/historypage', (req, res) => {
  filemgr
    .getAllData()
    .then(result => {
      filteredResults = result;
      console.log(filteredResults[0]);
      res.render('historypage.hbs');
    })
    .catch(errorMessage => {
      console.log(errorMessage);
    });
  ...
  for (var i = 0; i < length; i++) {
    if (originalResults[i].photos) {
      ...
      tempObj = {
        name: originalResults[i].name,
        address: originalResults[i].vicinity,
        photo_reference: 'noimageicon.jpg'
      };
    }
